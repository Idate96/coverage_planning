...
def find_connectivity(slice: np.array):
    ...
    """
    genus = 0  # num of disconnected parts of a line
    segments = []
...
def find_slices_adjacency(slice_low: List[Tuple[int, int]], slice_high: List[Tuple[int, int]]):
    ...
    for i, segment_low in enumerate(slice_low):
        for j, segment_high in enumerate(slice_high):
            ...
            len_overlapping_region = min(segment_high[1], segment_low[1]) - max(segment_high[0], segment_low[0])
...
def create_mask(binary_image: np.array):
    ...
    for col_id in range(binary_image.shape[1]):
        ...
        if num_previous_segments == 0:
            ...
            for j in range(num_segments):
                # merge event     
                if (np.sum(adj_matrix[:, j]) > 1 | np.sum(adj_matrix[:, j]) == 0): 
                    current_cells[j] = num_cells
                    num_cells += 1
            
            for i in range(len(current_cells)):
                mask[current_segments[i][0], current_segments[i][1]] = current_cells[i]
